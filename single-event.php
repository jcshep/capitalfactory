...
<?php
// Get event date and time fields
$event_date = get_field('date_time');
$end_time = get_field('end_time');

// Create DateTime objects if we have valid date/time values
$start_datetime = null;
$end_datetime = null;

if ($event_date) {
    $start_datetime = new DateTime($event_date . ' ' . $start_time);
}

if ($event_date && $end_time) {
    $end_datetime = new DateTime($event_date . ' ' . $end_time);
}
?>
    ...
    <div class="container">
        <div class="inside">
            <?php if (has_post_thumbnail()) : ?>
                <img src="<?= get_the_post_thumbnail_url(get_the_ID(), 'large') ?>" class="bg-img" alt="">
            <?php else : ?>
                <img src="<?php bloginfo('template_directory'); ?>/img/event-placeholder.jpg" class="bg-img" alt="Event Placeholder">
            <?php endif; ?>
            ...
            <div class="d-flex flex-wrap h-100 align-items-center justify-content-center text-center">
                <div class="col-12 p-45 py-0 d-flex flex-column justify-content-center align-items-center pb-5">
                    <?php if (get_field('event_type')): ?>
                        <div class="tag bg-white text-black"><?= get_field('event_type'); ?></div>
                        <div class="spacer-lg"></div>
                    <?php endif; ?>

                    <h1 class="display-xxl"><?= get_the_title(); ?></h1>
                    <div class="spacer-md"></div>
                    <div class="bg-cream rounded-corners-5 hero-date text-black heading-55 text-uppercase">
                        <?= $start_datetime ? $start_datetime->format('F j, Y') : ''; ?>
                        <br class="d-bloc d-sm-none">
                        @ <?= $start_datetime ? $start_datetime->format('g:i A') : ''; ?>
                        <?php if ($end_datetime): ?> - <?= $end_datetime->format('g:i A'); ?><?php endif; ?>
                    </div>
                </div>
            </div>
        </div>
                ...
                <p class="events-address d-flex flex-column gap-xxs text-gray">
                    ...
                    <?php if ($right['date_start']): ?>
                        <span class="d-flex align-items-center gap-xs">
                            ...
                            <span><?= date("l, F jS", $timestamp_start); ?> <?php if ($right['date_end'] && ($right['date_start'] !== $right['date_end'])) {
                                                                                echo ' - ' . date("l, F jS", $timestamp_end);
                                                                            } ?></span>
                        </span>
                    <?php endif; ?>
                    ...
                    <?php if ($right['time_start']): ?>
                        <span class="d-flex align-items-center gap-xs">
                            ...
                            <span><?= date("g:i a", $timestamp_start); ?> <?php if ($right['time_end'] && ($right['time_start'] !== $right['time_end'])) {
                                                                                echo ' - ' . date("g:i a", $timestamp_end);
                                                                            } ?></span>
                        </span>
                    <?php endif; ?>
                </p>
